...
#undef LISTNAME
	...
	},
	{
		...
		_T("刷新率"),
		_T("此选项可以调节游戏的刷新率（帧率）。"),
		_T("请在文本框内键入刷新率数值，若设为 0 则选择系统默认。若指定的刷新率不可用，游戏将自动选择一个最接近的刷新率。"),
	},
	{
		1, true, true,
		_T("game_zbufferbits"),
	...
	},
	{
		...
		_T("nocpk"),
		_T("免 CPK 补丁"),
		_T("原始情况下，游戏数据压缩存放于数个 CPK 文件之中。本选项可以让游戏直接从文件系统读取游戏数据，而不是从 CPK 中读取。"),
		NULL,
		NULL, NULL,
		{
			{ _T("禁用"), _T("禁用此选项后，游戏将从 CPK 中读取游戏数据。"), _T("0") },
			{ _T("启用"), _T("启用此选项后，游戏将直接从文件系统读取游戏数据（启用前，需要将 CPK 解压缩到正确位置）。"), _T("1") },
		}
	},
	{
		1, true, true,
		_T("console"),
		_T("调出控制台"),
		_T("原始情况下，若想调出游戏内置的控制台，需要先按 ~ 所在的按键，然后输入密码“")
#ifdef BUILD_FOR_PAL3
...
#endif
		_T("”，最后按回车。本选项可以允许直接调出控制台。"),
		NULL,
		NULL, NULL,
		{
			{ _T("禁用"), _T("禁用此选项后，需要输入密码才能调出控制台。"), _T("0") },
			{ _T("启用"), _T("启用此选项后，无需输入密码，直接按 ~ 所在的按键即可调出控制台。"), _T("1") },
		}
	},
	{
		1, true, true,
		_T("skipscreenshot"),
		_T("跳过截图"),
		_T("游戏系统内进行特定操作时，需要用到游戏画面的截图，但截图操作速度较慢，会造成一定卡顿。此选项可以控制是否用纯黑图片替代截图，以减少这类卡顿。该选项不会影响截屏快捷键 F8 的功能。"),
		NULL,
		NULL, NULL,
		{
	...
	},
	{
		...
		_T("screenshotformat"),
		_T("截屏文件格式"),
		_T("本选项可以指定截屏快捷键 F8 保存的图片文件格式（截屏文件保存在 snap 目录下）。"),
		NULL,
		NULL, NULL,
		{
			{ _T("BMP"), _T("截屏图片将会以 BMP 格式保存（截屏速度最快，画质无损，文件占用空间很大）。"), _T("bmp") },
			{ _T("PNG"), _T("截屏图片将会以 PNG 格式保存（截屏速度最慢，画质无损，文件占用空间较大）。"), _T("png") },
			{ _T("JPG"), _T("截屏图片将会以 JPG 格式保存（截屏速度适中，画质有损，文件占用空间较小）。当“图形设置——图形接口”选项设置为 9.0b 时，截图的画质较低；若想提高截图的画质，请选择 9.0c 图形接口。"), _T("jpg") },
		}
	},
	{
		1, true, true,
	...
	},
	{
		...
		NULL,
		_T("打开截屏目录"),
		_T("此功能可以打开游戏的截屏目录（游戏中按 F8 键可以截屏）。"),
