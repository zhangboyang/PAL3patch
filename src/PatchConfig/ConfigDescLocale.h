...
#undef LISTNAME
	...
	},
	{
		...
		_T("saveimg"),
		_T("存档图片"),
		_T("此选项可以控制是否保存存档对应的小截屏。禁用存档图片可以减少进入人物状态界面时的卡顿，但会导致存档文件没有对应的截屏。"),
		NULL,
		NULL,
		{
			{ _T("禁用"), _T("禁用此选项后，存档时不会保存游戏截屏，从而减少进入人物状态界面时的卡顿。"), _T("0") },
			{ _T("启用"), _T("启用此选项后，存档时会保存游戏截屏，但进入人物状态界面时可能会卡顿。"), _T("1") },
		}
	},
	{
		1, TRUE, TRUE,
	...
	},
	{
		...
		NULL,
		_T("官方配置工具"),
		_T("此功能可以打开官方编写的游戏配置工具。使用它可以编辑“音乐开关”、“音效开关”、“垂直同步”等选项。\r\n")
#ifdef BUILD_FOR_PAL3
		_T("注意：补丁的软件光标设置会覆盖官方配置工具中的同名设置。")
#endif
#ifdef BUILD_FOR_PAL3A
		_T("注意：补丁的分辨率设置、软件光标设置会覆盖官方配置工具中的同名设置。")
#endif
		,
