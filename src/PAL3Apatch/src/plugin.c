...
//   that was allocated in PAL3Apatch
...
void plugin_warning(const char *module, int indent, const char *fmt, ...)
{
    ...
    if (plugin_warning_msgboxes + 1 <= MAXWARNMSGBOXES) {
        ...
        try_goto_desktop();
        MessageBoxW(NULL, cs2wcs_managed(msgbuf, CP_UTF8, &msgbox_buf), L"PAL3Apatch Plugin Host", MB_ICONWARNING | MB_TOPMOST | MB_SETFOREGROUND);
    }
    free_msgbox_buf();
    va_end(ap);
}
...
void plugin_fail(const char *module, int indent, const char *fmt, ...)
{
    ...
    write_plugin_log(module, indent, msgbuf);
    try_goto_desktop();
    MessageBoxW(NULL, cs2wcs_managed(msgbuf, CP_UTF8, &msgbox_buf), L"PAL3Apatch Plugin Host", MB_ICONERROR | MB_TOPMOST | MB_SETFOREGROUND);
    free_msgbox_buf();
    die(2);
    va_end(ap);
}
...
#define pplog(fmt, ...) plugin_plog("PAL3APATCH", plugin_log_indent, fmt, ##__VA_ARGS__)
