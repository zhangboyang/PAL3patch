...
static HANDLE game_mutex = NULL, pal3_mutex = NULL;
static HANDLE temp_mutex = NULL;

static void acquire_game_mutex()
{
    if (!(game_mutex = acquire_mutex("PAL3Apatch_GameMutex", 100)) || !(pal3_mutex = acquire_mutex("PAL3patch_GameMutex", 100))) {
...
void fork_laa(int laa)
{
    ...
    HANDLE fork_mutex = acquire_mutex("PAL3Apatch_ForkMutex", 100);
    if (!fork_mutex) die(1);
    if (game_mutex) release_mutex(game_mutex);
    if (pal3_mutex) release_mutex(pal3_mutex);
...
{
    temp_mutex = acquire_mutex("PAL3Apatch_TempMutex", 100);
    if (!temp_mutex) die(1);
