...
#define PE_TEMPLATE_FILE "PAL3A.EXE"
#define PE_OUTPUT_FILE "PAL3Aunpacked.exe"
#define PE_TOTAL_SECTIONS 7
#define PE_IMAGEBASE 0x00400000
#define PE_CODEBASE 0x00001000
#define PE_SECTIONALIGN 0x1000
#define PE_FILEALIGN 0x1000
#define PE_ENTRY_RVA (0x5431C0 - PE_IMAGEBASE)
#define PE_IAT_RVA (0x00558000 - PE_IMAGEBASE)
#define PE_IMPORT_RVA (0x0229A000 + 0x1000 - PE_IMAGEBASE)
#define PE_IMPORT_DLLCOUNT 8
#define PE_IMPORT_SIZE ((PE_IMPORT_DLLCOUNT + 1) * sizeof(IMAGE_IMPORT_DESCRIPTOR))
#define S_EXTDATA_SIZE (12 * 0x1000)
...
unsigned udata_size; // SizeOfUninitializedData
...
void read_sdata()
{
    ...
    for (sid = 0; sid < PE_TOTAL_SECTIONS; sid++) {
        ...
        if (strcmp(sdata[sid].name, ".text") == 0) {
            ...
            assert(sid == 3);
            sdata[sid].flags = 0x40000040; // Initialized Data, Read Only
        } else if (strcmp(sdata[sid].name, ".exttext") == 0) {
            assert(sid == 4);
            sdata[sid].flags = 0x60000020; // Code, Execute, Read
        } else if (strcmp(sdata[sid].name, ".extrdata") == 0) {
            assert(sid == 5);
            sdata[sid].flags = 0x40000040; // Initialized Data, Read Only
        } else if (strcmp(sdata[sid].name, ".extdata") == 0) {
            assert(sid == 6);
            sdata[sid].flags = 0xC0000080; // Uninitialized Data, Read Write
        } else assert(0);
    }
    fclose(fp);
    ...
    for (sid = 0; sid < PE_TOTAL_SECTIONS; sid++) {
        char fn[MAXLINE];
        if (strcmp(sdata[sid].name, ".extrdata") == 0 || strcmp(sdata[sid].name, ".extdata") == 0) {    
            strcpy(fn, "NODATA");
            sdata[sid].vsize = sdata[sid].psize = 0;
        } else {
        ...
        if (strcmp(sdata[sid].name, ".text") == 0) {
            ...
            if (strcmp(sdata[sid].name, ".data") == 0) {
                ...
                assert(PE_IMPORT_RVA == sdata[sid].base);
            } else if (strcmp(sdata[sid].name, ".extdata") == 0) {
                udata_size += (sdata[sid].vsize = S_EXTDATA_SIZE);
            } 
        }
...
unsigned make_ntheader()
{
    ...
    popthdr->SizeOfUninitializedData = udata_size;
...
int main()
{
    ...
    *ppeheaderssize = peptr;
    ...
    peptr += make_sections();
    peptr = ROUND_UP(peptr, PE_FILEALIGN);
    *ppeimagesize = sdata[PE_TOTAL_SECTIONS - 1].base + sdata[PE_TOTAL_SECTIONS - 1].vsize;
